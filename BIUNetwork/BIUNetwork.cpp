...
SpikingNetwork::SpikingNetwork(const std::vector<int>& layerSizes, double vth, double vdd, int refractory, double cn, double ctotal, std::vector<double> weights) {
	for (int size : layerSizes) {
		layers.emplace_back(size, vth, vdd, refractory, cn, ctotal, weights);
	}
}
